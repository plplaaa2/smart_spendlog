...
#### A. 로컬 자동 백업 및 복원
- **자동 백업 스케줄링**: 설정 화면 내 **매일 밤 12시에 데이터 자동 백업 실행** 옵션을 활성화하면, 자정(KST 00:00)에 사용자별 가계부 SQLite 데이터베이스가 통째로 로컬 디스크 `/data/backups/`에 복사됩니다.
- **롤링 유지 정책**: 최근 7일 동안의 백업본만 보존되며, 7일이 지난 백업은 디스크 용량 확보를 위해 매일 밤 백업 시 자동 순환 삭제됩니다.
- **UI 일체형 복원 및 다운로드**: **설정 > 데이터 관리** 탭 하단의 백업 목록을 통해 원하는 백업 일시의 파일을 **즉시 복원(Restore)**하거나 브라우저를 통해 로컬 PC로 **즉시 다운로드(Download)** 및 **개별 삭제**할 수 있습니다.
  > [!IMPORTANT]
  > 백업본 복원 실행 시 기존 DB 커넥션 종료 → 백업본 덮어쓰기 → 커넥션 재바인딩 및 Home Assistant 실시간 센서 재동기화 흐름이 동적으로 일괄 진행됩니다.
...
- **설정 항목**:
  ...
  2. **네트워크 백업 경로**:
     ...
     * **리눅스 호스트**: 호스트 디바이스 상에 마운트된 로컬 경로 (예: `/share/backups`)를 입력합니다.
  3. **자격 증명 (선택사항)**:
