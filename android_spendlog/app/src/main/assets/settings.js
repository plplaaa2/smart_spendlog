...
async function loadSettingsTab() {
  try {
    ...
    // 알림 권한 상태 체크 (Android Bridge 연동)
    checkNotiPermissionStatus();
    checkPostNotiPermissionStatus();
...
async function loadMerchantCategories() {
  try {
    ...
    if (mcatSel) {
      ...
      const expenseCats = categoriesToUse.filter(c => c.type === 'EXPENSE');
...
async function loadDataSettings() {
  // 수동 데이터 백업 및 복원 탭으로 축소됨 (자동 네트워크 백업 기능 제거)
}
...
// 알림 권한 상태 체크 및 업데이트
async function checkNotiPermissionStatus() {
  if (window.AndroidBridge && typeof window.AndroidBridge.callApi === 'function') {
    try {
      const resStr = await window.AndroidBridge.callApi('api/permissions/notification', JSON.stringify({ method: 'GET' }));
      const res = JSON.parse(resStr);
      const isGranted = res.body && res.body.granted === true;

      updateNotiStatusUI(isGranted);
    } catch (e) {
      console.error('권한 상태 확인 실패:', e);
    }
  }
}

function updateNotiStatusUI(isGranted) {
  const dot = document.getElementById('noti-status-dot');
  const text = document.getElementById('noti-status-text');
  if (!dot || !text) return;

  if (isGranted) {
    dot.className = 'status-indicator online';
    text.textContent = '권한 활성화됨';
    text.className = 'text-success';
  } else {
    dot.className = 'status-indicator offline';
    text.textContent = '권한 비활성화됨';
...
function openNotificationSettings() {
  if (window.AndroidBridge && typeof window.AndroidBridge.callApi === 'function') {
    window.AndroidBridge.callApi('api/permissions/notification/request', JSON.stringify({ method: 'POST' }));

    // 설정 화면에서 돌아왔을 때를 대비해 주기적으로 체크 (임시)
    setTimeout(checkNotiPermissionStatus, 2000);
    setTimeout(checkNotiPermissionStatus, 5000);
  } else {
    alert('이 기능은 안드로이드 앱 환경에서만 작동합니다.');
  }
}

// 앱 알림 권한 상태 체크
async function checkPostNotiPermissionStatus() {
  if (window.AndroidBridge && typeof window.AndroidBridge.callApi === 'function') {
    try {
      const resStr = await window.AndroidBridge.callApi('api/permissions/post_notification', JSON.stringify({ method: 'GET' }));
      const res = JSON.parse(resStr);
      const isGranted = res.body && res.body.granted === true;

      updatePostNotiStatusUI(isGranted);
    } catch (e) {
      console.error('앱 알림 권한 확인 실패:', e);
    }
  }
}

function updatePostNotiStatusUI(isGranted) {
  const dot = document.getElementById('post-noti-status-dot');
  const text = document.getElementById('post-noti-status-text');
  if (!dot || !text) return;
...
function requestPostNotificationPermission() {
  if (window.AndroidBridge && typeof window.AndroidBridge.callApi === 'function') {
    window.AndroidBridge.callApi('api/permissions/post_notification/request', JSON.stringify({ method: 'POST' }));

    setTimeout(checkPostNotiPermissionStatus, 2000);
    setTimeout(checkPostNotiPermissionStatus, 5000);
  } else {
    alert('이 기능은 안드로이드 앱 환경에서만 작동합니다.');
  }
}
...
// 배터리 최적화 제한 제외 상태 체크
async function checkBatteryPermissionStatus() {
  if (window.AndroidBridge && typeof window.AndroidBridge.callApi === 'function') {
    try {
      const resStr = await window.AndroidBridge.callApi('api/permissions/battery', JSON.stringify({ method: 'GET' }));
      const res = JSON.parse(resStr);
      const isIgnoring = res.body && res.body.granted === true;

      updateBatteryStatusUI(isIgnoring);
    } catch (e) {
      console.error('배터리 권한 확인 실패:', e);
    }
  }
}

function updateBatteryStatusUI(isIgnoring) {
  const dot = document.getElementById('battery-status-dot');
  const text = document.getElementById('battery-status-text');
  if (!dot || !text) return;

  if (isIgnoring) {
    dot.className = 'status-indicator online';
    text.textContent = '제한 해제됨 (안정적)';
    text.className = 'text-success';
  } else {
    dot.className = 'status-indicator offline';
    text.textContent = '제한 적용됨 (불안정)';
    text.className = 'text-danger';
  }
}
...
function requestBatteryOptimizationPermission() {
  if (window.AndroidBridge && typeof window.AndroidBridge.callApi === 'function') {
    window.AndroidBridge.callApi('api/permissions/battery/request', JSON.stringify({ method: 'POST' }));

    setTimeout(checkBatteryPermissionStatus, 2000);
    setTimeout(checkBatteryPermissionStatus, 5000);
  } else {
    alert('이 기능은 안드로이드 앱 환경에서만 작동합니다.');
  }
}
