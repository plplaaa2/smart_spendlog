...
async function loadIncomeAnalytics(year, month) {
  ...
  try {
    ...
    if (trendTitleEl) {
      // 버그 수정: 함수 파라미터 year가 undefined일 경우 targetYear 폴백 값을 사용하도록 수정
      trendTitleEl.textContent = isYearly ? `수입 월별 추이 (${targetYear}년)` : '수입 월별 추이 (최근 6개월)';
    }
    ...
    if (res.trend && res.trend.length > 0) {
      // 요약: 실제 수입 데이터가 존재하는 개월 수를 반영해 월 평균 소득 계산
      // 의존성: MainActivity.kt의 analytics/income API가 반환하는 trend 데이터 형식과 밀접하게 연관됨
      const sum = res.trend.reduce((acc, cur) => acc + (cur.total || 0), 0);
      if (isYearly) {
        const activeMonths = res.trend.filter(cur => (cur.total || 0) > 0).length;
        averageIncome = Math.round(sum / (activeMonths || 1));
      } else {
        averageIncome = Math.round(sum / res.trend.length);
      }
    }
