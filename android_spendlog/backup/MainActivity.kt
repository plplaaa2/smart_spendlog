...
class MainActivity : ComponentActivity() {
    ...
    companion object {
        ...
        fun refreshUI() {
            instance?.runOnUiThread {
                instance?.webView?.evaluateJavascript("if(typeof refreshCurrentTabData === 'function') refreshCurrentTabData(); if(typeof loadLogs === 'function') loadLogs();", null)
            }
        }
    }
    ...
    private fun setupWebView(webView: WebView) {
        ...
        // WebChromeClient 설정: <select> 드롭다운 네이티브 피커, alert/confirm/prompt 다이얼로그 지원
        // 의존성: index.html 내 모든 <select> 요소 및 JS의 alert()/confirm()/prompt() 호출과 연결
    ...
    inner class AndroidBridge {
        private fun queryRaw(sql: String, args: Array<out Any?>? = null): JsonArray {
        ...
        private fun queryRawSingle(sql: String, args: Array<out Any?>? = null): JsonObject? {
            return queryRaw(sql, args).firstOrNull()?.jsonObject
        }
        ...
        fun callApi(url: String, optionsJson: String): String {
            ...
            return kotlinx.coroutines.runBlocking {
                val result = handleApiRequest(url, method, body)
        ...
        private suspend fun handleApiRequest(url: String, method: String, body: String?): ApiResponse {
            try {
                ...
                // Kotlin Serialization 직렬화 시 기본값(예: Category.type = EXPENSE) 생략을 방지하여 웹뷰 필터링 오류를 예방함
                // 의존성: assets/app.js의 updateCategorySelect() 내 카테고리 type 필터링과 연결
                val json = Json { ignoreUnknownKeys = true; encodeDefaults = true }
                
                return when {
                    // 주의: merchant_categories, package_pay_methods가 categories, pay_methods보다 먼저 매칭되어야 함
                    // 의존성: app.js의 loadMetadata(), settings.js의 loadMerchantCategories()/loadPackagePayMethods()와 연결
                    path.startsWith("merchant_categories") -> {
                        if (path == "merchant_categories/seed-presets" && method == "POST") {
                            val count = seedFranchisePresets(db)
                    ...
                    path == "pay_methods" || path.startsWith("pay_methods/") -> {
                        ApiResponse(body = json.encodeToJsonElement(db.payMethodDao().getAllPayMethods()))
                    }
                                    ...
                                    try {
                                        db.runInTransaction {
                                            kotlinx.coroutines.runBlocking {
                                                ...
                                                try {
                                                    // pay_methods 복원 시 type 필드 자동 판별 처리:
                                                    // 오리지널 account_book 백업에는 type 컬럼이 없으므로,
                                                    // type 필드 누락 시 이름 기반으로 BANK/CARD 자동 판정하여
                                                    // 은행이 CARD로 잘못 복원되는 버그를 방지합니다.
                                                    // 의존성: Entities.kt의 PayMethod(name, type) 및 Daos.kt의 PayMethodDao와 연결
                                                ...
                                                try {
                                                    // rules 복원: merchant_template 누락 가능성 대비 null-safe 처리
                                                    // 의존성: Entities.kt의 Rule 엔티티 및 Daos.kt의 RuleDao와 연결
                                                ...
                                                try {
                                                    // transactions 복원: 원본 account_book에서 memo, raw_text가 NULL일 수 있으므로
                                                    // json.decodeFromJsonElement() 대신 수동 파싱으로 null-safe 처리.
                                                    // used_point NULL → 0L, memo/raw_text NULL → "" 로 폴백합니다.
                                                    // 의존성: Entities.kt의 Transaction 엔티티 및 Daos.kt의 TransactionDao와 연결
                    ...
                    path.startsWith("settings") -> {
                        if (method == "POST" && body != null) {
                            ...
                            val resBody = buildJsonObject {
                                ...
                                // AI 필드 전송 (JS 'true'/'false' 동치 비교 대응)
                    ...
                    path.startsWith("transactions") -> {
                        if (method == "POST" && body != null) {
                            // 수동 JSON 파싱: 프론트엔드에서 id=""(신규) 또는 숫자 id(수정)를 모두 허용
                            // package 필드는 Transaction 엔티티에 없으므로 무시하며, null-safe 처리 적용
                            // 의존성: app.js의 transaction-form submit 이벤트 및 Entities.kt의 Transaction 엔티티와 연결
                            val bodyObj = json.parseToJsonElement(body).jsonObject
                            val idRaw = bodyObj["id"]?.jsonPrimitive?.contentOrNull
                            val idLong = idRaw?.toLongOrNull() ?: 0L  // 빈문자열("") 또는 null → 0 (autoGenerate)
                            val tx = Transaction(
                                id = idLong,
                                type = bodyObj["type"]?.jsonPrimitive?.contentOrNull ?: "EXPENSE",
                                amount = bodyObj["amount"]?.jsonPrimitive?.longOrNull ?: 0L,
                                merchant = bodyObj["merchant"]?.jsonPrimitive?.contentOrNull ?: "",
                                category = bodyObj["category"]?.jsonPrimitive?.contentOrNull ?: "",
                                payMethod = bodyObj["pay_method"]?.jsonPrimitive?.contentOrNull ?: "",
                            ...
                            val resultList = queryRaw(sql, params.toTypedArray())
                    ...
                    path.startsWith("rules") -> {
                        if (path == "rules/ai-generate" && method == "POST" && body != null) {
                            ...
                            val type = json.parseToJsonElement(body).jsonObject["type"]?.jsonPrimitive?.contentOrNull ?: "EXPENSE"
                    ...
                    path == "parse-test" && method == "POST" && body != null -> {
                        val jsonObject = json.parseToJsonElement(body).jsonObject
                        val text = json.parseToJsonElement(body).jsonObject["text"]?.jsonPrimitive?.content ?: ""
                        val pattern = json.parseToJsonElement(body).jsonObject["pattern"]?.jsonPrimitive?.content ?: ""
                        val category = json.parseToJsonElement(body).jsonObject["category"]?.jsonPrimitive?.content ?: ""
                        val payMethod = json.parseToJsonElement(body).jsonObject["pay_method"]?.jsonPrimitive?.content ?: ""
                        val type = json.parseToJsonElement(body).jsonObject["type"]?.jsonPrimitive?.contentOrNull ?: "EXPENSE"
                    ...
                    path.startsWith("permissions/notification") -> {
                        if (path.endsWith("request")) {
                            ...
                            val flat = android.provider.Settings.Secure.getString(applicationContext.contentResolver, "enabled_notification_listeners")
                    ...
                    path == "analytics/compare" -> {
                        ...
                        if (mode == "mom") {
                            ...
                            val cal = Calendar.getInstance()
                            cal.set(Calendar.YEAR, year.toIntOrNull() ?: 2026)
                    ...
                    path == "analytics/fixed" -> {
                        ...
                        val totalSpentRow = queryRawSingle(
                        ...
                        val fixedTotalRow = queryRawSingle(
                        ...
                        val trendArray = buildJsonArray {
                            if (isYearly) {
                                val trendRows = queryRaw(
                                    "SELECT strftime('%Y-%m', datetime) as month, SUM(amount) as total FROM transactions WHERE datetime LIKE ? AND type = 'EXPES' AND category IN ($placeholders) GROUP BY month ORDER BY month ASC",
                                    arrayOf(targetPattern, *fixedCategories.toTypedArray())
                                )
                    ...
                    path == "analytics/general" -> {
                        ...
                        val generalTotalRow = queryRawSingle(
                    ...
                    path == "analytics/income" -> {
                        ...
                        val incomeTotalRow = queryRawSingle(
                    ...
                    // merchant_categories, package_pay_methods 라우트는 위로 이동됨 (categories/pay_methods startsWith 충돌 방지)
        ...
        private suspend fun seedFranchisePresets(db: SpendLogDatabase): Int {
            return try {
                val presets = com.spendlog.android.parser.FranchisePresets.loadPresets(applicationContext)
