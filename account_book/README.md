...
- **해외 결제(USD) 대응**: 해외 카드 결제(달러) 수동 결제 및 문자 자동 파싱을 지원하고 실시간 환율 API 연동 원화 환산 및 실청구액 입력을 통한 적용 환율 역산/보정 기능을 제공합니다.
- **지능형 AI 연동 및 패턴 생성**: Gemini/OpenAI API를 통한 파싱 실패 시 AI 폴백 분석, 미분류 메시지에 대한 정규식 패턴 자동 생성 추천 및 시각화 그래프가 결합된 프리미엄 AI 소비 분석 리포트(v2.4.0)를 지원합니다.
...
- **자동 분류 학습 및 가맹점 프리셋**: 230개 이상의 가맹점 프리셋 사전에 기반해 카테고리를 자동 학습 및 재분류하며, 결제수단과 결제방식(신용/체크)을 완전히 독립 필드로 관리합니다. 또한 포인트/지원금 사용액을 카드 실적 및 지출에 자동으로 합산(v2.4.2)합니다.
- **반응형 웹 대시보드 & 통계**: 모바일 및 태블릿에 최적화된 반응형 2행 헤더 레이아웃을 제공하며 일반/고정/수입지출 분석을 통한 다각도 통계 화면을 지원합니다.
...
### 4. AI 분석 및 정규식 패턴 생성 설정 방법

지능형 소비 리포트와 미분류 거래의 정규식 패턴 자동 완성을 위해 AI API 연동 설정을 제공합니다.

- **지원 서비스**: Gemini API (권장), OpenAI API 또는 OpenAI 호환 로컬 LLM API를 연동할 수 있습니다.
- **설정 항목**:
  1. **AI 서비스 활성화**: 서비스 웹 UI 내 **설정 > AI 분석 설정** 탭에서 기능을 활성화합니다.
  2. **API Key**: Gemini 혹은 OpenAI에서 발급받은 API 키를 입력합니다. (키는 AES-256-CBC 알고리즘으로 양방향 암호화되어 안전하게 보관됩니다.)
  3. **선택 모델**: `gemini-1.5-flash` (기본값), `gpt-4o-mini` 등 연동할 LLM 모델명을 기입합니다.
  4. **통신 타임아웃**: 고부하 마크업 생성 지연에 방어하기 위해 통신 타임아웃 제한은 **180초(3분)**로 적용되어 있으며, 대기 텍스트가 표시됩니다.
- **주요 기능 활용**:
  * **AI 소비 분석 리포트**: 대시보드 내 **AI 분석 리포트** 탭에서 생성 버튼을 누르면 최근 소비 통계를 기반으로 다크 모드에 최적화된 시각화 그래프가 결합된 종합 보고서가 즉시 출력됩니다.
  * **AI 패턴 생성 추천**: 가계부 내 **미분류 거래 알림 로그** 상세 화면에서 **AI 패턴 생성** 버튼을 누르면 정규식 패턴 및 결제수단/결제방식이 자동 추출되어 신규 규칙 폼에 자동 매핑됩니다.
