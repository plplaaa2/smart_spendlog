...
- **반응형 2행 헤더 및 모바일 레이아웃 최적화 (v2.2.1 신규)**:
  - 데스크탑 및 모바일 환경에서 헤더를 타이틀 행과 컨트롤 행으로 나누어 복잡도를 낮추고 모바일 뷰포트 드롭다운을 한 줄로 컴팩트하게 배치했습니다.
  - 가계부 카드의 타이틀과 서브 설명이 모바일에서 충돌하지 않도록 `.card-header` 세로 스택 정렬 시스템을 보강했습니다.
  - 대시보드의 최근 지출 내역 카드 더 보기 버튼을 소형 규격(`btn-sm`)으로 컴팩트하게 축소 조정했습니다.
