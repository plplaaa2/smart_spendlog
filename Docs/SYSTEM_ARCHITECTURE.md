...
4. 활성 규칙은 `priority ASC, id ASC`로 조회하며, `parseNotification()`은 우선순위 순서대로 검사해 처음으로 유효하게 파싱된 결과를 반환한다.
...
| `account_book/parser/pattern_validator.js` | AI 생성 정규식의 저장 전 안전성 검증 |
| `account_book/parser/auto_rule_builder.js` | 로컬 자동 규칙 생성·안전성 검사·원문 재파싱 |
