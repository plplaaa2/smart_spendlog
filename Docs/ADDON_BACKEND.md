...
- 자동 규칙 생성이 활성화된 재시도는 최초 파싱 실패 후 검증된 `AUTO` 규칙을 저장하고, 활성 규칙을 다시 조회하여 재파싱한 뒤 거래와 로그를 갱신하는 흐름까지 통합 테스트한다.
- 생성된 `AUTO` 규칙이 원본 알림 재파싱에 실패하면 해당 규칙을 즉시 삭제하고 기존 거래와 실패 로그를 보존한다.
